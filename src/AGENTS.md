...
1. **Shell execution is always `Bun.$`**, never `child_process.exec`/`spawn`. See `utils.ts:installBinary` and `core.ts` for the pattern: `await $\`sh -c ${cmd}\`.quiet()`. Quote interpolated arguments naturally — `Bun.$` escapes them.
...
5. **Every `install*` / `deploy*` function returns `InstallResult`** — never throw to the caller for expected failures; return `{ status: "failed", verifyPassed: false, message }` instead. Only `core.ts`'s outer `try/catch` throws (after restoring the backup).
...
- **Backup before overwrite.** `core.ts` calls `createBackup(paths)` before the first deploy and `restoreFromPartialManifest(backup)` in the `catch`. Any new core step must add its target path to the `backupPaths` array in `installCore`.
...
- **Hook scripts get `chmod 700`**, not `+x`. See `core.ts:deployHooks`. `verify.ts:verifyHooks` checks the executable bit across `0o111`.
...
### Adding a New Core Step

1. Write a `private async function deployX(env, dryRun): Promise<InstallResult>` in `core.ts`.
2. Append the target path(s) to the `backupPaths` array in `installCore`.
...
| `node:fs/promises` | `utils.ts`, `backup.ts`, `core.ts` | `mkdir`, `exists`, `cp`, `symlink`, `readdir`, `stat` (Bun has no native replacements for these) |
| `node:path` | multiple | `join`, `dirname` |

### Internal Module Imports

- `types.ts` is imported by every other file (type-only).
- `utils.ts` is imported by `core.ts`, `components/*.ts`, `commands/setup.ts`, `detect.ts`, `verify.ts`.
- `detect.ts` is imported by `commands/setup.ts` and `commands/status.ts`.
- `backup.ts` is imported by `core.ts` and `restore.ts`.
- `core.ts`, `verify.ts`, `status.ts`, `restore.ts` are imported only by `commands/*.ts`.

...
1. **Tier 1 — user-global base** (`~/.claude/skills/`): always-on baseline. Contains our 17 custom skills (`do`, `ship-feature`, `fix-bug`, `refactor-safely`, `security-audit`, `onboard-codebase`, `tdd-first`, `doc-hygiene`, `ci-hygiene`, `coding-style`, `research-first`, `knowledge-base`, `loop-patterns`, `release-cut`, `setup`, `team-do`, `worktree-task`) + skills.sh seed bundle (`find-skills`, `caveman`, `karpathy-guidelines`, `playwright-cli`). If a match exists here, use it.

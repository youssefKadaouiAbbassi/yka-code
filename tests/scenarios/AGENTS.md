...
1. **Temp-dir only for filesystem writes.** Every spec that writes files must use `mkdtemp(join(tmpdir(), "yka-code-..."))` and `rm(tmpDir, { recursive: true, force: true })` in `afterEach`. Copy the pattern from `primordial.test.ts` lines 78-88.
